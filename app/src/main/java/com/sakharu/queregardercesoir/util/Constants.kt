package com.sakharu.queregardercesoir.util

/*
    CATEGORIES
 */
const val CATEGORY = "category"

const val CATEGORY_POPULAR_NAME = "Ils ont le vent en poupe"
const val CATEGORY_POPULAR_ID = 1L
const val CATEGORY_POPULAR_OVERVIEW = "Les films les plus populaires du moment. Actualisé chaque jour."

const val CATEGORY_TOPRATED_NAME="Ils ont conquis le public"
const val CATEGORY_TOPRATED_ID=2L
const val CATEGORY_TOPRATED_OVERVIEW="Les film les mieux notés par le public toutes générations confondues."

const val CATEGORY_NOWPLAYING_NAME="En salle en ce moment"
const val CATEGORY_NOWPLAYING_ID=3L
const val CATEGORY_NOWPLAYING_OVERVIEW="Les films actuellement en salle près de chez vous."

const val CATEGORY_UPCOMING_NAME="Bientôt en salle"
const val CATEGORY_UPCOMING_ID=4L
const val CATEGORY_UPCOMING_OVERVIEW="Les fims prochainement en salle près de chez vous."

/*
    ACTIONS
 */
const val ACTION_LOAD_MORE_CATEGORY_DETAIL="ActionLoadMoreCategoryDetail"
